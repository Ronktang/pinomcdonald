...
function print_apk_info {
    ...
    PACKAGE_NAME="$(echo "${AAPT_OUTPUT}" | sed -En "1 s/.*\Wname='([^']*)'.*/\1/p")"
    VERSION_CODE="$(echo "${AAPT_OUTPUT}" | sed -En "s/.*\WversionCode='([^']*)'.*/\1/p")"
    VERSION_NAME="$(echo "${AAPT_OUTPUT}" | sed -En "s/.*\WversionName='([^']*)'.*/\1/p")"
